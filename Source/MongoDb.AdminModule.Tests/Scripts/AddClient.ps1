...
New-Client -AbsoluteRefreshTokenLifetime 10 -AccessTokenLifetime 20 -AccessTokenType Reference -EnableLocalLogin $false -AllowRememberConsent $true -AuthorizationCodeLifetime 30 -ClientId test -ClientName unittest -Enabled $true -Flow AuthorizationCode -IdentityProviderRestrictions restriction1, restriction2 -IdentityTokenLifetime 40 -LogoUri uri:logo -PostLogoutRedirectUris uri:logout1,uri:logout2 -RedirectUris uri:redirect1, uri:redirect2 -RefreshTokenExpiration Sliding -RefreshTokenUsage ReUse -RequireConsent $true -AllowedScopes openid,email,roles -SlidingRefreshTokenLifetime 50 -ClientSecrets $clientSecrets -AlwaysSendClientClaims $true -IncludeJwtId $true -PrefixClientClaims $true -AllowedCustomGrantTypes grantrestriction1, grantrestriction2, grantrestriction3 -Claims $clientClaims -AllowClientCredentialsOnly $true -UpdateAccessTokenClaimsOnRefresh $true -AllowedCorsOrigins cors1,cors2,cors3 -AllowAccessToAllScopes $true -AllowAccessToAllCustomGrantTypes $true -AllowAccessTokensViaBrowser $false -LogoutSessionRequired $false -RequireSignOutPrompt $false -LogoutUri somelogouturi | Set-Client -connection $connection -Database $database